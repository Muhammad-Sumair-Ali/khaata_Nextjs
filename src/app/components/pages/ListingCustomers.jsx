...
const ListingCustomers = ({ setCustomerActive,customerActive }) => {
  ...
  const formatCustomerBalance = (balance) => {
    if (balance < 0) {
      return `Rs. ${Math.abs(balance)} dene hai`;
    } else if (balance > 0) {
      return `Rs. ${balance} lene hain!`;
    } else {
      return (
        <span className="text-red-700">
          Rs. 00 <span className="text-blue-700 text-sm font-mono">Clear</span>
        </span>
      );
    }
  };
  ...
  return (
    <>
      ...
      <ul className="overflow-y-auto h-[calc(100vh-80px)] p-2 space-y-3 -mt-2">
        ...
        {data && Array.isArray(data) && data.map((customer) => (
          <li
              ...
              <div className="flex gap-2">
                <div
                  className={`text-lg rounded-xl h-14 flex items-center justify-center px-2 ${
                    customer.totalGive - customer.totalGet < 0 ? "text-red-500" : "text-blue-700"
                  }`}
                >
                  <strong className="block">
                    {formatCustomerBalance(customer.totalGive - customer.totalGet)}
                  </strong>
                </div>
              </div>
            </div>
